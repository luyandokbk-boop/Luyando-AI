...
# ===== CONFIG =====
HF_TOKEN = "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" # <-- PUT YOUR TOKEN HERE
...
def set_bg(image_file):
    try:
        ...
        st.markdown(f"""<style>.stApp {{background-image: url("data:image/png;base64,{encoded}"); background-size: cover;}}.voice-card {{background: linear-gradient(135deg, rgba(239,125,0,0.3), rgba(0,149,67,0.3)); border: 3px solid {ZAMBIA_COLORS['orange']}; border-radius: 20px; padding: 30px;}}</style>""", unsafe_allow_html=True)
    except: pass
...
# ===== STYLING =====
st.markdown(f"""<style>h1 {{color: {ZAMBIA_COLORS['orange']}; text-align: center; font-size: 52px; font-weight: 900;}}.stButton>button {{background: linear-gradient(90deg, {ZAMBIA_COLORS['orange']}, {ZAMBIA_COLORS['green']}); color: white; font-weight: 900; width: 100%; border-radius: 15px; height: 60px; font-size: 20px;}}</style>""", unsafe_allow_html=True)
...
if not st.session_state.logged_in:
    ...
    if st.button("LOGIN"):
        ...
        if c.fetchone(): st.session_state.logged_in = True; st.session_state.user = user; st.rerun()
        else: st.error("Try: admin / admin123")
else:
    ...
    with tab1:
        ...
        if uploaded_file and st.button("🚀 TURN MY VOICE INTO SONG") and credits > 3:
            if music_client:
                with st.spinner("🎼 AI is listening to your voice and creating song... 3 minutes"):
                    try:
                        ...
                        prompt = f"Professional {genre} song in {mood} mood. Use vocals similar to uploaded voice. Full instrumental, beat, mixing, mastering."
                        audio = music_client.text_to_audio(prompt, max_new_tokens=1024)
                        
                        st.success("✅ YOUR SONG IS READY!")
                        st.audio(audio)
                        st.download_button("⬇️ DOWNLOAD YOUR SONG", audio, "my_voice_song.wav")
                ...
                st.error("Add HF_TOKEN in line 14")
        elif credits <= 3: 
            st.error("Voice to Song costs 3 credits")
        st.markdown('</div>', unsafe_allow_html=True)

    # TAB 2: PRO GENERATOR
    with tab2:
        st.header("⚡ PRO SONG GENERATOR")
        pro_prompt = st.text_area("Describe your song:", placeholder="A worship song in Bemba")
